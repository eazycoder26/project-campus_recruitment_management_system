...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($result->num_rows > 0) {
        ...
        $update_sql = "UPDATE company_reg SET password = ? WHERE email = ?";
        $update_stmt = $con->prepare($update_sql);
        $update_stmt->bind_param("ss", $new_password, $email);
        
        if ($update_stmt->execute()) {
            // Redirect to the login page after successful password change
            header("Location: /EUPHORIA/project_campus_management/campus_recruitment_management_system/login_company.php");
            exit();
        } else {
